...
**成果物**:
- 仕様の明確化（順序が必要か否か）
- 必要な場合: `backend/src/importer/detector.rs`の修正

**タスク**:
- [ ] 仕様書を確認し、road_typesと角度の対応関係が必要かを判断
- [ ] 順序が必要な場合: HashSetをVecに変更し、角度計算時の順序を保持
- [ ] 順序が不要な場合: 現状維持、ドキュメントに明記

**完了条件**:
- road_typesの仕様が明確になっている
- 必要な場合は実装が修正されている
- テストで順序が正しいことを確認
